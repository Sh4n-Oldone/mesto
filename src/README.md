...
* Создание классов, работа с ООП, рефакторинг v.5.0
* Список веток
...
- [x] Перенести константы в отдельный файл
- [x] Создать класс Section
  - [x] Использовать класс для добавления новых и исходных карточек
- [x] Создать класс Popup
  - [x] Разобраться с методом _handleEscClose()
  - [x] Повесить на переменную, в которой записан класс слушатели через метод класса Popap
- [x] Создать класс PopupWithImage
- [x] Изменить Card чтобы он принимал функцию открытия попапа картинки из класса PopupWithImage
- [x] Создать класс PopupWithForm
- [x] Создать класс UserInfo
- [x] Удалить функции, убрать логи, удалить ненужные переменные
- [x] Работает!
- [x] Настроить Вебпак
- [x] Отправить на первое ревью
- [x] Отправить на второе ревью
...
- Фикс клика по оверлею для закрытия попапа.
- Исправлено обнуление инпутов профиля.

**Создание классов, работа с ООП, рефакторинг v.5.0**

- Классы перенесены в директорию components.
- index.js перенесён в pages.
- Все переменные перенесены в utils/constants.js
- Изменён принцип работы с классом Section. Метод render теперь тоже используется.
- Ссылка в ReadMe исправлена на ссылку репозитория (после сборки github-pages больше не может корректно обрабатывать страницу)
- Сам ReadMe пока решено оставить как есть. Некуда больше контроль над прогрессом сложить =(
- Метод render вызывается отдельно от записи в переменную, а новая карточка создаётся через метод addItem уже готовою копии section.
